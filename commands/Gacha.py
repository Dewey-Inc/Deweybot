...
class GachaCog(commands.cog.GroupCog, name="gacha"):
    ...
    @discord.app_commands.command(name="help", description="What is a gacha?")
    async def gacha_help(self, ctx : discord.Interaction):
    ...
    @discord.app_commands.command(name="viewcard", description="View a gacha card!")
    async def gacha_viewcard(self, ctx : discord.Interaction, id: int, show:bool=False):
        if not Permissions.banned(ctx):
    ...
    #@discord.app_commands.command(name="browsecards", description="Look through cards")
    #async def gacha_browsecards(self, ctx : discord.Interaction, page:int = 1):
    #    if not Permissions.banned(ctx):
    ...
    @discord.app_commands.command(name="submitcard", description="Submit a new gacha card!")
    async def gacha_submitcard(self, ctx : discord.Interaction, name: str, description: str, image: discord.Attachment, additional_info:str=""):
    ...
    @discord.app_commands.command(name="self-submissions", description="View cards you submitted")
    async def gacha_browsecards(self, ctx : discord.Interaction, page:int = 1):
        if not Permissions.banned(ctx):
            if page <= 0: page = 1
    ...
    @discord.app_commands.command(name="editcard", description="Re-submit an edited gacha card (or admin)!")
    async def gacha_editcard(self, ctx : discord.Interaction, id: int, name: str = "", description: str = ""):
        if not Permissions.check_if_in_main_guid(ctx=ctx): 
            await ctx.response.send_message("You can only run this if you're in the main server!", ephemeral=True)
            return
        if not Permissions.banned(ctx):
    ...
    @discord.app_commands.command(name="stats", description="card stats")
    async def gacha_stats(self, ctx : discord.Interaction, user: discord.Member | discord.User | None = None):
        if not Permissions.banned(ctx):
    ...
    @discord.app_commands.command(name="stats-spread", description="card spread")
    async def gacha_stats_spread(self, ctx : discord.Interaction):
        if not Permissions.banned(ctx):
            embed = discord.Embed(title="Statistics", description="WIP, i was just curious abpuit these :) stats :)")
            spread = {}
    ...
    @discord.app_commands.command(name="stats-accepted-spread", description="accepted card spread")
    async def gacha_stats_accepted_spread(self, ctx : discord.Interaction):
        if not Permissions.banned(ctx):
            embed = discord.Embed(title="Statistics", description="WIP, i was just curious abpuit these :) stats :)")
            spread = {}
    ...
    async def gacha_inventory(self, ctx : discord.Interaction, show: bool=True, view_button: bool=False):
        if not Permissions.banned(ctx):
    ...
    @discord.app_commands.command(name="inventory-completion", description="View your progress in collecting!")
    async def gacha_inventory_completion(self, ctx : discord.Interaction):
        if not Permissions.banned(ctx):
    ...
    async def gacha_roll(self, ctx : discord.Interaction):
        if not Permissions.check_if_in_main_guid(ctx=ctx): 
            await ctx.response.send_message("You can only run this if you're in the main server!", ephemeral=True)
            return
        if not Permissions.banned(ctx):
    ...
    if Bot.DeweyConfig["deweycoins-enabled"]:
        @discord.app_commands.command(name="sellcard", description="Sell a card!")
        async def gacha_sell_card(self, ctx : discord.Interaction, card_id : int, quantity : int = 1):
    ...
    {card.name + ("'" if quantity == 1 else "'s")} for D¢{gachalib.getCardCost(card=card) * quantity}", ephemeral=False,view=view)
        ...
        @discord.app_commands.command(name="sellall", description="Sell ALL of a card!")
        async def gacha_sell_all(self, ctx : discord.Interaction, card_id : int):
            success, card = gachalib.cards.get_card_by_id(card_id=card_id)
    ...
    {card.name + ("'" if user_cards == 1 else "'s")} for D¢{gachalib.getCardCost(card=card) * len(user_cards)}", ephemeral=False,view=view)
        ...
        @discord.app_commands.command(name="selldupes", description="Sell all DUPLICATES of a card!")
        async def gacha_sell_all_dupes(self, ctx : discord.Interaction, card_id : int):
            success, card = gachalib.cards.get_card_by_id(card_id=card_id)
    ...
    {card.name + ("'" if quantity == 1 else "'s")} for D¢{gachalib.getCardCost(card=card) * quantity}", ephemeral=False,view=view)
        ...
        @discord.app_commands.command(name="buypack", description="Buy a special PACK of Deweycards")
        async def gacha_buy_pack(self, ctx : discord.Interaction):
            layout = gachalib.views.buy_packs.BuyPackView(ctx.user)
            await ctx.response.send_message(view=layout, ephemeral=True)

        @discord.app_commands.command(name="valuecard", description="Check the value of a card in DeweyCoin")
        async def gacha_get_price(self, ctx : discord.Interaction):
    ...
    @discord.app_commands.command(name="trade", description="Trade with someone")
    async def gacha_trade(self, ctx : discord.Interaction, user:discord.Member|discord.User):
        if not Permissions.check_if_in_main_guid(ctx=ctx): 
            await ctx.response.send_message("You can only run this if you're in the main server!", ephemeral=True)
            return
        if not Permissions.banned(ctx):
    ...
    #@discord.app_commands.command(name="send-card", description="Give someone a card")
    #async def gacha_send_card(self, ctx : discord.Interaction, inv_id:int, user:discord.Member):
    #    if not Permissions.check_if_in_main_guid(ctx=ctx): 
    #        await ctx.response.send_message("You can only run this if you're in the main server!", ephemeral=True)
    #        return
    ...
    @discord.app_commands.command(name="roll-reminders", description="Enable/disable gacha rolling reminders")
    async def gacha_settings_roll_reminders(self, ctx : discord.Interaction, set:bool):
        if not Permissions.banned(ctx):
    ...
    @discord.app_commands.command(name="z-admin-deletecard", description="!MOD ONLY! (Ask us!) Delete a card")
    @discord.app_commands.check(predicate=Permissions.admin_check)
    async def z_gacha_admin_deletecard(self, ctx : discord.Interaction, id:int):
        gachalib.cards.delete_card(id)
        await ctx.response.send_message("Deleted card.", ephemeral=True)


    @discord.app_commands.command(name="z-admin-approvecard", description="!MOD ONLY! Force an action on a card (use when buttons don't work)")
    @discord.app_commands.check(predicate=Permissions.gacha_approve_check)
    async def z_gacha_admin_approvecard(self, ctx : discord.Interaction, id:int, approved: bool):
        success,card = gachalib.cards.get_card_by_id(id)
        if success:
    ...
    @discord.app_commands.command(name="z-admin-givecard", description="!MOD ONLY! Just give someone a card")
    @discord.app_commands.check(predicate=Permissions.admin_check)
    async def z_gacha_admin_givecard(self, ctx : discord.Interaction, id:int, user:discord.Member|discord.User):
    ...
    @discord.app_commands.command(name="z-admin-setrarity", description="!MOD ONLY! Set the rarity of a card")
    @discord.app_commands.check(predicate=Permissions.gacha_approve_check)
    async def z_gacha_admin_setrarity(self, ctx : discord.Interaction, id:int, rarity:gachalib.Rarities):
        success,card = gachalib.cards.get_card_by_id(id)
        if success:
    ...
    @discord.app_commands.command(name="z-admin-unapproved-cards", description="!MOD ONLY! See all non-approved cards")
    @discord.app_commands.check(predicate=Permissions.gacha_approve_check)
    async def z_gacha_admin_unapproved_cards(self, ctx : discord.Interaction):
