...
class GodCog(Bot.DeweyCog, name="kfad"):
    ...
    @discord.app_commands.command(name="help", description="What is a 'god' and why for a day?")
    async def gfad_help(self, ctx : discord.Interaction):
        if not Permissions.banned(ctx):
            await ctx.response.send_message(content="Test!", ephemeral=True)


    @discord.app_commands.command(name="z-roll", description="! ADMIN ONLY ! Roll GOD 🎲🎲🎲🎲🎲🎲🎲🎲🎲🎲🎲🎲🎲🎲🎲🎲🎲🎲🎲")
    async def gfad_roll(self, ctx : discord.Interaction, message_requirement:int = -1, days:int = 7, exclude_previous_gods:bool=False):
    ...
    @discord.app_commands.command(name="z-get-qualifiers", description="! ADMIN ONLY ! Get people who qualify")
    async def gfad_get_qualifiers(self, ctx : discord.Interaction, message_requirement:int = -1, exclude_prev_gods:bool=True, days:int = 7):
        if Permissions.check_permission(ctx=ctx, permission=Permissions.PERMISSION_ADMIN):
            if message_requirement == -1: message_requirement = Bot.DeweyConfig["kfad-must-have"]
            range_now = datetime.datetime.today()
            range_start = range_now - datetime.timedelta(days=days)
