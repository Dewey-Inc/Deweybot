...
class DeweycoinCog(Bot.DeweyCog, name="deweycoin"):
    ...
    @discord.app_commands.command(name="wallet", description="View your wallet!")
    async def money_wallet(self, ctx : discord.Interaction, show: bool=True, user: discord.User | discord.Member | None = None):
    ...
    @discord.app_commands.command(name="stats", description="View your stats!")
    async def money_stats(self, ctx : discord.Interaction, show: bool=True, user: discord.User | discord.Member | None = None):
        if user == None: user = ctx.user
    ...
    if Bot.DeweyConfig["obs-integration-enabled"]:
        @discord.app_commands.command(name="buy-image", description="Show an image on stream with D¢100")
        async def buy_image(self, ctx : discord.Interaction, image : discord.Attachment):
    ...
    @discord.app_commands.command(name="doors", description="Gamble with 3 doors")
    async def gambling_doors(self, ctx : discord.Interaction, bet: int):
    ...
    @discord.app_commands.command(name="coins", description="Gamble with many many many coins!")
    async def gambling_coins(self, ctx : discord.Interaction, coins: int):
        if not Permissions.check_if_in_main_guid(ctx=ctx): 
            await ctx.response.send_message("You can only run this if you're in the main server!", ephemeral=True)
            return
    ...
    @discord.app_commands.command(name="givecoins", description="Give coins to someone else")
    async def money_give_coin(self, ctx : discord.Interaction, user: discord.Member | discord.User, coins:int):
        if user == None: user = ctx.user
        if coins <= 0:
    ...
    @discord.app_commands.command(name="z-movecoins", description=" ! ADMIN ONLY ! force move coins from user -> user (ex. take from dewey) (allows debt)")
    @discord.app_commands.check(predicate=Permissions.admin_check)
    async def gacha_z_move_coin(self, ctx : discord.Interaction, from_user:discord.Member | discord.User, to_user: discord.Member | discord.User | None, coins:int):
    ...
    @discord.app_commands.command(name="z-givecoins", description=" ! ADMIN ONLY ! materialize coins (i advise against doing this)")
    @discord.app_commands.check(predicate=Permissions.admin_check)
    async def money_z_give_coin(self, ctx : discord.Interaction, user: discord.Member | discord.User | None, coins:int):
        if user == None: user = ctx.user
        moneylib.giveCoins(user.id, coins)
        await ctx.response.send_message("ok",ephemeral=True)


    @discord.app_commands.command(name="z-audit", description=" ! ADMIN ONLY ! how much money exists in the wild")
    @discord.app_commands.check(predicate=Permissions.admin_check)
    async def money_z_audit(self, ctx : discord.Interaction, show:bool=True):
